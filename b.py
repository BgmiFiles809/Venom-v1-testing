...
def broadcast_message(message):
    ...
    bot.reply_to(message, response)
# Command handler for setting cooldown
...
def set_cooldown_command(message):
    ...
    if user_id in admin_id:
        ...
        if len(command_info) == 3:
            ...
            try:
                cooldown_time = int(command_info[2])
                if cooldown_time < 0:
                    raise ValueError
                set_cooldown(user_id, command, cooldown_time)
                response = f"Cooldown for command '{command}' set to {cooldown_time} seconds."
            except ValueError:
                response = "Please provide a valid cooldown time (in seconds)."
        else:
            response = "Usage: /setcooldown <command> <seconds>"
    else:
        response = "You do not have permission to use this command."

    log_command(user_id, '/setcooldown', response)
    bot.reply_to(message, response)

# Command handler for bgmi command with cooldown
@bot.message_handler(commands=['bgmi'])
def bgmi_command(message):
    ...
    if user_id not in allowed_user_ids:
        ...
        log_command(user_id, '/bgmi', response)
        bot.reply_to(message, response)
        return

    # Check if the command is on cooldown
    if is_on_cooldown(user_id, '/bgmi'):
        response = "This command is on cooldown. Please wait before using it again."
        log_command(user_id, '/bgmi', response)
        bot.reply_to(message, response)
        return

    # Set a cooldown for the command (e.g., 60 seconds)
    set_cooldown(user_id, '/bgmi', 60)
    
    # Process the bgmi command (this part can be customized)
    response = "bgmi command processed successfully."
